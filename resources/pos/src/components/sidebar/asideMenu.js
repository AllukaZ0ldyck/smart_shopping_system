...
import { faAngleDown, faBars, faSearch, faStore } from "@fortawesome/free-solid-svg-icons";
...
const AsideMenu = (props) => {
    ...
    const { stores, loginUser } = useSelector(state => state);
    ...
    useEffect(() => {
        if (stores.length > 0) {
            setSelectedStore(stores?.find((store) => store?.attributes?.active));
        }
                    ...
                    <DropdownButton
                        ...
                        title={
                            <div className="d-flex align-items-center gap-2">
                                <FontAwesomeIcon icon={faStore} className="fs-1 text-primary" />
                                <div className="d-flex flex-column">
                                    <span className="text-muted small text-start">{getFormattedMessage("store.header.name.title")}</span>
                                    <span className="fw-semibold text-primary text-truncate store-name text-start">
                                        {selectedStore ? selectedStore?.attributes?.name : ""}
                                    </span>
                                </div>
                                <FontAwesomeIcon icon={faAngleDown} className="fs-4 text-secondary px-2" />
                            </div>
                        }
                        variant=""
                        className={`${isMenuCollapse
                                ? "d-none"
                                : "py-0 m-2 store-dropdown rounded text-center fw-semibold"
                            }`}
                    >
                        {stores
                            ...
                            .map((store, index) => (
                                <Dropdown.Item
                                    ...
                                    className="py-1 px-4 fs-6 d-flex align-items-center gap-3"
                                    active={false}
                                >
                                    <input
                                        type="radio"
                                    ...
                                    />
                                    <span className={`mb-0 ${selectedStore?.id == store.id ? 'text-primary fw-bold' : ''}`}>{store.attributes.name}</span>
                                </Dropdown.Item>
                            ))}
                    </DropdownButton>
                    {/* <a
